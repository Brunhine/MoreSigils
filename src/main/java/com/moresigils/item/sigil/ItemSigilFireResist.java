...
public class ItemSigilFireResist extends ItemSigilBaseToggleable implements IPotionSigil
{
    private PotionEffect potionEffect;
    private Potion mobEffect;

    public ItemSigilFireResist()
    {
        super("FireResist", ConfigHandler.sigilFireResistCost);
        MinecraftForge.EVENT_BUS.register(this);

        ItemStack stack = PotionUtils.addPotionToItemStack(new ItemStack(Items.POTIONITEM), PotionTypes.FIRE_RESISTANCE);
        this.potionEffect = PotionUtils.getEffectsFromStack(stack).get(0);

        this.mobEffect = MobEffects.FIRE_RESISTANCE;
    }
