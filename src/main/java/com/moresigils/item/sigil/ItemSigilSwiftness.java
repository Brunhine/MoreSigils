...
public class ItemSigilSwiftness extends ItemSigilBaseToggleable implements IPotionSigil
{
    private final PotionEffect potionEffect;
    private final Potion mobEffect;

    public ItemSigilSwiftness()
    {
        super("Swiftness", ConfigHandler.sigilSwiftnessCost);
        MinecraftForge.EVENT_BUS.register(this);

        ItemStack stack = PotionUtils.addPotionToItemStack(new ItemStack(Items.POTIONITEM), PotionTypes.SWIFTNESS);
        this.potionEffect = PotionUtils.getEffectsFromStack(stack).get(0);

        this.mobEffect = MobEffects.SPEED;
    }
