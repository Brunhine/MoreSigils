...
public class ItemSigilStrength extends ItemSigilBaseToggleable implements IPotionSigil
{
    private final PotionEffect potionEffect;
    private final Potion mobEffect;

    public ItemSigilStrength()
    {
        super("Strength", ConfigHandler.sigilStrengthCost);
        MinecraftForge.EVENT_BUS.register(this);

        ItemStack stack = PotionUtils.addPotionToItemStack(new ItemStack(Items.POTIONITEM), PotionTypes.STRENGTH);
        this.potionEffect = PotionUtils.getEffectsFromStack(stack).get(0);

        this.mobEffect = MobEffects.STRENGTH;
    }
