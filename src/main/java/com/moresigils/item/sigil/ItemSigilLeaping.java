...
public class ItemSigilLeaping extends ItemSigilBaseToggleable implements IPotionSigil
{
    private PotionEffect potionEffect;
    private Potion mobEffect;

    public ItemSigilLeaping()
    {
        super("Leaping", ConfigHandler.sigilLeapingCost);
        MinecraftForge.EVENT_BUS.register(this);

        ItemStack stack = PotionUtils.addPotionToItemStack(new ItemStack(Items.POTIONITEM), PotionTypes.LEAPING);
        this.potionEffect = PotionUtils.getEffectsFromStack(stack).get(0);

        this.mobEffect = MobEffects.JUMP_BOOST;
    }
